...
Reconstruction of the Cherenkov angle for kaons (top) and pions (bottom) at 6 GeV/c with a $125^{\circ}$ polar angle using the design parameters above are shown in Figure \ref{fig:EIC_track_res}. The signal is very clean and the mean and SPR of the distribution are easily extracted. Figure \ref{fig:EIC_performance}a shows the photon yield, or multiplicity, per polar angle for 50 6 GeV/c pions, and Figure \ref{fig:EIC_performance}b shows the SPR per polar track for 50 6 GeV/c kaons (red) and pions (blue). The per track Cherenkov angle resolution, given by (\ref{eq:performance}), is shown in Figure \ref{fig:EIC_track_res} for assumptions of 0.25 mrad (black), 0.5 mrad (red), 0.75 (green), and 1 mrad (blue) correlated term contributions with 6 GeV/c pions \footnote{NB: the per track Cherenkov angle resolution will be slightly different for each particle species, however because the SPR for each particle is almost identical showing only the results for pions is sufficient.}.  The simulations were done assuming that the sides of the 3-layer lens focusing optic were not reflective, therefore reducing the photon yield and making the performance slightly worse.

The SPR of the reconstructed Cherenkov angle was found to scale roughly linearly with the size of the pixel size of the MCP-PMTs, as shown in Figure \ref{fig:EIC_sensor_scaling}. Clearly the 4 mm pixel size, though not ideal, is comparable in performance to the 2 mm pixel size. This is an important factor to consider in the final design due to the rapid increase of the cost per pixel of MCP-PMTs as the size of the pixels decreases.

Overall these results show that due to the design's large expansion volume, small pixel size, and ease of signal reconstruction the performance of this design can reach the desired performance for the required physics.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[scale=0.6]{EIC_SPR_vs_sensorDim.png}
